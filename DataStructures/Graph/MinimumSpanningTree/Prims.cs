...
namespace DataStructures.Graph.MinimumSpanningTree
{
        ...
        where TW : IComparable
    {
        public List<MSTEdge<T,TW>> FindMinimumSpanningTree(IGraph<T> graph)
        {
            ...
            dfs(graph, graph.ReferenceVertex, new Heap.BinaryHeap<MSTEdge<T, TW>>(Shared.SortDirection.Ascending),
                new HashSet<T>(),edges);
        ...
        private void dfs(IGraph<T> graph, IGraphVertex<T> currentVertex, BinaryHeap<MSTEdge<T, TW>> spNeighours, HashSet<T> spVertices, List<MSTEdge<T, TW>> spEdges)
        {
            while (true)
            {
                foreach(var edge in currentVertex.Edges)
                {
                    ...
                    spNeighours.Add(new MSTEdge<T, TW>(currentVertex.Key, edge.TargetVertexKey, edge.Weight<TW>())); 
                }
